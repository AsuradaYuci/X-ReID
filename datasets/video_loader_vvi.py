...
def pil_loader_lmdb(path):
    buf = six.BytesIO()
    buf.write(path)
    buf.seek(0)
    imgs0 = Image.open(buf).convert('RGB')
    return imgs0


def read_image_lmdb(img_path):
    """Keep reading image until succeed.
    This can avoid IOError incurred by heavy IO process."""
    got_img = False
    while not got_img:
...
class VideoDataset_train_lmdb(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            imgs_ir_seq = self.transform(imgs_ir_seq)

        imgs_ir = torch.stack(imgs_ir_seq[0], dim=0)  # torch.Size([10, 3, 288, 144])
        ...
        imgs_rgb_seq = [rgb_seq]
        if self.transform is not None:
            imgs_rgb_seq = self.transform(imgs_rgb_seq)

        imgs_rgb = torch.stack(imgs_rgb_seq[0], dim=0)
        ...
        if self.sample == 'dense':
            ...
            for indices in indices_list:
                ...
                for index in indices:
                    index = int(index)
                    img_path = img_paths[index]  # img = read_image_lmdb(img_path)
#                     img = read_image_lmdb(img_path)
                    img = read_image(img_path)
                    # 添加
                    img = np.array(img)
