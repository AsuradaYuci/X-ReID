...
def decoder_pic_path(fname):  # '0001M2D3T1F1.jpg'
    base = fname[0:4]  # 0001
    modality = fname[5]  # 2
    if modality == '1':
        modality_str = 'ir'
...
class VCM(object):
    root1 = '/YCY/dataset/VCM/'
    root2 = '/data/HITSZ-VCM/'
    root3 = 'dataset/VCM/'
    ...
    def _process_data_train(self, names, meta_data, relabel=False, min_seq_len=0):
        ...
        num_tracklets = meta_data.shape[0]  # 11061
        pid_list = list(set(meta_data[:, 3].tolist()))  # [1,2,...,500]
        num_pids = len(pid_list)  # 500
        ...
        for tracklet_idx in range(num_tracklets):
            ...
            m, start_index, end_index, pid, camid = data  # [2, 1, 24, 1, 3]
            # cams.append(camid)
            ...
            if m == 1:  # IR modality
                ...
                if len(img_ir_paths) >= min_seq_len:  # Filter out samples with low frame rates
                    ...
                    ir_label.append(pid)
                    tracklets_ir.append((img_ir_paths, pid, camid-1))
                    # same id
                ...
                if len(img_rgb_paths) >= min_seq_len:
                    ...
                    tracklets_rgb.append((img_rgb_paths, pid, camid-1))
                    # same id
