...
class TPM_space_shift(nn.Module):
    ...
    def forward(self, x, B, T):  # x: torch.Size([40, 163, 768])
        ...
        x_patch = x[:, 1:, :].permute(0, 2, 1)  # patch tokens  torch.Size([40, 162, 768])  torch.Size([40, 768, 162])
        x_patch_split = x_patch.view(B, T, D, -1)  # b,T,D,l torch.Size([4, 10, 768, 162])
        x_patch_shift = self.shift(x_patch_split).view(B*T, D, -1).permute(0, 2, 1)  # torch.Size([40, 162, 768])
        #
        # x_fuse1 = torch.cat((x_cls.unsqueeze(1), x_patch_shift), dim=1)  # torch.Size([40, 163, 768])
        x_fuse_intra = self.Reconstuct_shift(x_cls, x_patch_shift)  # torch.Size([40, 163, 768])
        # x_fuse_intra2 = x_fuse_intra.unsqueeze(1)  # 30, 1, 768
        x_fuse_intra2 = self.Reconstuct_shift2(x_fuse_intra, x_patch_shift)
        return x_fuse_intra2
    @staticmethod
